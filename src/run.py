...
argp.add_argument(
    "--tb_expt_name", help="debug string for tb log.", default="run"
)
...
if args.function == "pretrain":
    ...
    ### YOUR CODE HERE ###
    # Load the model if pretrained parameters are provided
    ...
    with open(args.outputs_path, "w", encoding="utf-8") as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path, encoding="utf-8")):
            ...
            x = x + "⁇"
            x = torch.tensor(
                [pretrain_dataset.stoi[s] for s in x], dtype=torch.long
            )[None, ...].to(device)
